...
2. **论文模式（正式入口）**：在项目根执行 `uv run python main.py`（或安装后 `uv run visio-agent`），终端列出 `input/` 下用户并选择，读 `.docx`，抽取 **3.2.1 数据库概念设计** 与 **第四章**，固定 `user` 后跑 Agent（ER + 时序 + 流程）。可选环境变量 `VISIO_PROJECT_ROOT` 指定项目根绝对路径。
3. **快速测试**：`uv run python examples.py er` / `sequence` / `flowchart`（内置短话术），或 `examples.py "<自然语言>"`；无参数会提示用法并退出
